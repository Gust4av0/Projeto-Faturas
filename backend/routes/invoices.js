...
router.post("/", async (req, res) => {
  try {
    ...
    const stmt = db.prepare(
      "INSERT INTO invoices (id, title, description, folderId, status) VALUES (?, ?, ?, ?, ?)",
    );
    stmt.run(
      invoiceId,
      title.trim(),
  ...
  try {
    ...
    if (invoiceId) {
      ...
      const updateStmt = db.prepare(
        "UPDATE invoices SET pdfPath = ?, originalPdfPath = ?, password = ?, status = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?",
      );
      updateStmt.run(
        outputPath,
        originalPdfPath,
    ...
    const autoTitle =
      fileNameWithoutExt || `Fatura ${new Date().toLocaleDateString("pt-BR")}`;

    const stmt = db.prepare(
      "INSERT INTO invoices (id, title, originalPdfPath, password, folderId, status) VALUES (?, ?, ?, ?, ?, ?)",
    );
    stmt.run(
      invoiceIdToUse,
      autoTitle,
    ...
    if (error.code === "PASSWORD_TOOL_UNAVAILABLE") {
      ...
      return res.status(503).json({
        ...
        error:
          "O servidor identificou que o PDF tem senha, mas o runtime de PDF protegido nao esta pronto. Rode npm install no backend ou em backend/pdf-runtime, reinicie o backend e tente novamente.",
      });
    }
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updateStmt = db.prepare(
      "UPDATE invoices SET title = ?, description = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?",
    );
    updateStmt.run(
      title || invoice.title,
